...
class DisciplinesRepository {
  ...
  Future<List<Map<String, Object?>>> getPendingMaps() async {
    final db = await _db;
    final rows = await db.rawQuery("SELECT * FROM disciplines WHERE sync_state != 'synced' OR (remote_id IS NULL AND deleted = 0)");
    return rows;
  }

  Future<Map<String, Object?>?> findByRemoteId(String remoteId) async {
    final db = await _db;
    final rows = await db.rawQuery('SELECT * FROM disciplines WHERE remote_id = ? LIMIT 1', [remoteId]);
  ...
  Future<void> setRemoteId(int localId, String remoteId) async {
    final db = await _db;
    await db.update('disciplines', {'remote_id': remoteId, 'sync_state': 'synced'}, where: 'id = ?', whereArgs: [localId]);
  }

  Future<void> markSyncedByLocalId(int localId) async {
    final db = await _db;
    await db.update('disciplines', {'sync_state': 'synced'}, where: 'id = ?', whereArgs: [localId]);
  }

  Future<void> applyRemoteToLocal(Map<String, dynamic> remote) async {
    final db = await _db;
    final existing = await db.rawQuery('SELECT id, updated_at FROM disciplines WHERE remote_id = ? LIMIT 1', [remote['id']]);
    final now = DateTime.now().millisecondsSinceEpoch;
    final row = {
      'name': remote['name'],
      'teacher': remote['teacher'],
      'group_code': remote['group_code'],
    ...
    if (existing.isEmpty) {
      await db.insert('disciplines', row..remove('id'));
    } else {
      final localId = existing.first['id'] as int;
      await db.update('disciplines', row, where: 'id = ?', whereArgs: [localId]);
    }
  }
}
