              ...
              child: Container(
                ...
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      const Text('Создать группу', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
                      const SizedBox(height: 16),
                      Row(children: [
                        Expanded(child: TextField(controller: _nameCtrl, decoration: const InputDecoration(labelText: 'Название'))),
                        const SizedBox(width: 16),
                        Expanded(child: TextField(controller: _specialtyCtrl, decoration: const InputDecoration(labelText: 'Специальность'))),
                      ]),
                      ...
                      Row(children: [
                        ...
                        const SizedBox(width: 16),
                        Expanded(child: TextField(controller: _sizeCtrl, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Количество студентов'))),
                      ]),
                      const SizedBox(height: 12),
                      Row(children: [
                        Expanded(child: TextField(controller: _courseCtrl, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Курс'))),
                        const SizedBox(width: 16),
                      ...
                      Row(
                        ...
                        children: [
                          const Text('Назначения предметов (преподаватель, часы, даты, подгруппа)', style: TextStyle(fontWeight: FontWeight.w600)),
                        ...
                        return Container(
                          ...
                          child: Row(children: [
                            ...
                            IconButton(
                              ...
                              onPressed: () async {
                                final updated = await showDialog<GroupSubjectTeacher>(
                                  context: context,
                                  builder: (ctx) => _AssignmentDialog(
                                    disciplines: _allDisc.where((d) => d.id != null).toList(),
                                    teachers: _teachers.where((t) => t.id != null).toList(),
                                ...
                                if (updated != null) {
                                  setState(() => _assignments[idx] = updated);
                                }
                              },
                            ),
                            IconButton(
...
class _AssignmentDialogState extends State<_AssignmentDialog> {
  ...
  void _submit() {
    ...
    if (_disciplineId == null || _teacherId == null || hours <= 0) {
      try { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Заполните дисциплину, преподавателя и часы'))); } catch (_) {}
      return;
    }
    ...
    return Dialog(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Добавить назначение', style: TextStyle(fontWeight: FontWeight.bold)),
            const SizedBox(height: 12),
            DropdownButtonFormField<int>(
              decoration: const InputDecoration(labelText: 'Дисциплина'),
              isExpanded: true,
              value: _disciplineId,
              items: [for (final d in widget.disciplines) if (d.id != null) DropdownMenuItem<int>(value: d.id, child: Text(d.name))],
            ...
            DropdownButtonFormField<int>(
              ...
              isExpanded: true,
              value: _teacherId,
              items: [for (final t in _filteredTeachers) if (t.id != null) DropdownMenuItem<int>(value: t.id, child: Text(t.fullName))],
              onChanged: (v) => setState(() => _teacherId = v),
            ),
            const SizedBox(height: 8),
            TextFormField(controller: _hoursCtrl, decoration: const InputDecoration(labelText: 'Часы'), keyboardType: TextInputType.number),
            const SizedBox(height: 8),
            DropdownButtonFormField<String>(
              ...
              onChanged: (v) => setState(() => _subgroup = v ?? 'all'),
            ),
            const SizedBox(height: 8),
            Row(children: [
